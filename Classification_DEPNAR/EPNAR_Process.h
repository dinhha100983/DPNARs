...
public: 
	...
	void rulessort();
	void rulessort_for_var_selection();
	void printpolularRules();
	void printvalidRules();
	void printvalidNotRedundantRules();
	void printData();
	bool selectedrule(vector<unsigned int> InputTrans,epnAR* selectedrule, int* Y_value, bool NotReDun);
...
private:
	...
	vector<epnAR> sortrules_by_support(vector<epnAR> Rules);
