...
vector<epnAR> EPNAR_Process::sortrules(vector<epnAR> Rules)
{
	...
	for (unsigned int i = 0; i < Rules.size(); i++)
	{
		for (unsigned int j = Rules.size() - 1; j>i; j--)
		{
			...
			unsigned int r_antecedent_length = Rules[j].pos.size() + Rules[j].neg.size();
			unsigned int s_antecedent_length = Rules[j-1].pos.size() + Rules[j-1].neg.size();
			...
			else
			{
				if (((double)r_supp / double(r_conf) == (double)s_supp / double(s_conf)))
				{
					if (r_antecedent_length > s_antecedent_length)
					{
						//doi cho
						epnAR temp = Rules[j - 1];
						Rules[j - 1] = Rules[j];
					...
					else if(r_antecedent_length == s_antecedent_length & r_supp > s_supp)
					{
						//doi cho
						epnAR temp = Rules[j - 1];
						Rules[j - 1] = Rules[j];
...
void EPNAR_Process::rulessort_for_var_selection()
{
	this->validRules = sortrules_by_support(validRules);
	this->validNotRedundantRules = sortrules_by_support(validNotRedundantRules);
}

vector<epnAR> EPNAR_Process::sortrules_by_support(vector<epnAR> Rules)
{
	/* sap xep cac luat trong vetor<> Rules theo thu tu uu tien la do ho tro sau do den do tin cay*/
	// su dung giai thuat noi bot
	for (unsigned int i = 0; i < Rules.size(); i++)
	{
		for (unsigned int j = Rules.size() - 1; j > i; j--)
		{
			...
			{
				if ((r_supp == s_supp) &((double)r_supp / double(r_conf) >(double)s_supp / double(s_conf)))
				{
					//doi cho
					epnAR temp = Rules[j - 1];
					Rules[j - 1] = Rules[j];
...
bool EPNAR_Process::selectedrule(vector<unsigned int> InputTrans, epnAR* selectedrule, int* Y_value, bool NotReDun)
{
	if (NotReDun) {
		// su dung luat not redundant
		for (unsigned int i = 0; i < this->validNotRedundantRules.size(); i++)
		{
			if (ruletransmatching(&validNotRedundantRules[i], InputTrans))
			{
				selectedrule = &validNotRedundantRules[i];
				if (validNotRedundantRules[i].precedent)
					*Y_value = 1;
				else
					*Y_value = 0;
				return true;
